...
export default class Entity {
  ...
  createTimeline(opts) {
    this._timeline = this._app._vtween.newTimeLine(opts);
    // this._timeline = this._app.createObject('Timeline', {});
  }

  addTracks(target, props, opts) {
    if (this._timeline === null) {
      console.warn('adding tracks failed, please create timeline first.');
      return;
    }

    if (target === 'Entity') {
      let tracks = this._app._vtween.newTracks(this, props, opts);
      this._timeline.addTracks(tracks);
      return;
    }

    let com = this.getComp(target);

    if (!com) {
      return;
    }

    let tracks = this._app._vtween.newTracks(com, props, opts);
    this._timeline.addTracks(tracks);
  }
