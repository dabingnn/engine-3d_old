...
export default class DrawMng {
  ...
  tick() {
    ...
    this._rects.forEach(item => {
      ...
      this._lineBatchModel2D.addLine(
        _v3_tmp.set( item.x, item.y, 0.0),
        _v3_tmp2.set( item.x, item.y + item.h, 0.0),
        item.color
      );
      this._lineBatchModel2D.addLine(
        _v3_tmp.set( item.x, item.y + item.h, 0.0),
        _v3_tmp2.set( item.x + item.w, item.y + item.h, 0.0),
        item.color
      );
      this._lineBatchModel2D.addLine(
        _v3_tmp.set( item.x + item.w, item.y + item.h, 0.0),
        _v3_tmp2.set( item.x + item.w, item.y, 0.0),
        item.color
      );
      this._lineBatchModel2D.addLine(
        _v3_tmp.set( item.x + item.w, item.y, 0.0),
        _v3_tmp2.set( item.x, item.y, 0.0),
        item.color
      );
    ...
    this._axesList.forEach(item => {
      ...
      if (item.is2D) {
        this._lineBatchModel2D.addLine(item.pos, item.up, _c3_tmp.set( 1, 0, 0));
        this._lineBatchModel2D.addLine(item.pos, item.right, _c3_tmp.set( 0, 1, 0));
        this._lineBatchModel2D.addLine(item.pos, item.forward, _c3_tmp.set( 0, 0, 1));
      } else if (item.depthTest) {
        this._lineBatchModel.addLine(item.pos, item.up, _c3_tmp.set( 1, 0, 0));
        this._lineBatchModel.addLine(item.pos, item.right, _c3_tmp.set( 0, 1, 0));
        this._lineBatchModel.addLine(item.pos, item.forward, _c3_tmp.set( 0, 0, 1));
      } else {
        console.warn('We have not support it yet');
  ...
  addSphere(pos, radius, color, duration = 0.0, depthTest = true) {
    ...
    primitive.model._node.lscale.set( radius, radius, radius);
