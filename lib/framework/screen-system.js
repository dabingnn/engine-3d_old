...
function walk(node, enterCallback, exitCallback) {
  node.children.forEach(function (child) {
    let continueWalk = true;
    if (enterCallback) {
      continueWalk = enterCallback(child, node);
    }

    if (continueWalk !== false) {
      walk(child, enterCallback, exitCallback);
    }

    exitCallback && exitCallback(child, node);
  });
}
  ...
  tick() {
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      walk(screenRoot, entity => {
        ...
        if (maskComp) {
          maskComp.width = width;
          maskComp.height = height;
          this._stencilMgr.enterLevel();
          // todo add setup stencil state
          if (maskComp._setUpRenderData) {
            maskComp._setUpRenderData.setStencil(gfx.DS_FUNC_NEVER, this._stencilMgr.stencilRef, this._stencilMgr.stencilWriteMask, gfx.STENCIL_OP_REPLACE,
              gfx.STENCIL_OP_KEEP, gfx.STENCIL_OP_KEEP, this._stencilMgr.stencilWriteMask);
            screenRenderer.addItem(maskComp._setUpRenderData);
          }
        }
        ...
        let maskComp = entity.getComp('Mask');
        if (maskComp) {
          maskComp._clearRenderData.setStencil(gfx.DS_FUNC_NEVER, 0, this._stencilMgr.stencilWriteMask, gfx.STENCIL_OP_REPLACE,
            gfx.STENCIL_OP_KEEP, gfx.STENCIL_OP_KEEP, this._stencilMgr.stencilWriteMask);
          screenRenderer.addItem(maskComp._clearRenderData);
