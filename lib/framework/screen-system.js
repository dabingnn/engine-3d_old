...
class _ScreenRendererHelper {
  ...
  addItem(renderData) {
    let positions = renderData.positions;
    let uvs = renderData.uvs;
    let color = renderData.color;
    let node = renderData._node;
    ...
    if (renderData.vertexCount + this._vertexOffset > _vertsInIAPool || renderData.indexCount + this._indexOffset > _indicesInIAPool) {
      this._flush();
    }
    ...
    for (let i = 0; i < renderData.indexCount; ++i) {
      this._batchedIndices[this._indexOffset + i] = renderData._indices[i] + this._vertexOffset;
    }
    renderData._ia = ia;
    renderData._multipliedTransform = true;
    this._batchedItems.push(renderData);
    ia._vertexBuffer = this._vb;
    ia._indexBuffer = this._ib;
    ia._start = this._indexOffset;
    ia._count = renderData.indexCount;
    this._vertexOffset = this._vertexOffset + renderData.vertexCount;
    this._indexOffset = this._indexOffset + renderData.indexCount;
  }
  ...
  _submit(renderData) {
    ...
    model.setNode(renderData._multipliedTransform ? this._dummyNode : renderData._node);
    model.addEffect(renderData._effect);
  ...
  tick() {
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      utils.walk(screenRoot, entity => {
        ...
        if (spriteComp) {
          spriteComp._renderData.updateModelData();
          spriteComp._renderData.sortKey = sortKey++;
          spriteComp._renderData._viewID = view._id;
          screenRenderer.addItem(spriteComp._renderData);
        }

        let labelComp = entity.getComp('Label');
        if (labelComp) {
          labelComp._renderData.updateModelData();
          labelComp._renderData.sortKey = sortKey++;
          labelComp._renderData._viewID = view._id;
          screenRenderer.addItem(labelComp._renderData);
        }
        return true;
      });
