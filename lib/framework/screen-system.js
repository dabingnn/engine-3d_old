...
class _ScreenRendererHelper {
  ...
  addItem(renderData) {
    ...
    if (renderData.getVertexCount() === 0 || renderData.getIndexCount() === 0) {
      return;
    }
    ...
    renderData._ia = ia;
    renderData._multiplyTransform = true;
  ...
  _flush() {
    ...
    for (let i = 0; i < this._batchedItems.length; ++i) {
      ...
      if (nextItem && nextItem._batchKey === item._batchKey) {
        // do batch
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      utils.walk(screenRoot, entity => {
        ...
        if (spriteComp) {
          ...
          for (let i = 0; i < spriteComp._renderData.getRenderDataCount(); ++i) {
            screenRenderer.addItem(spriteComp._renderData.getRenderData(i));
          }
        }
        ...
        if (labelComp) {
          ...
          for (let i = 0; i < labelComp._renderData.getRenderDataCount(); ++i) {
            screenRenderer.addItem(labelComp._renderData.getRenderData(i));
          }
        }
        return true;
      });
