  ...
  tick() {
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      if (
        ...
        let ent = _getWidgetAt(screen, this._entities, input.mouseX, input.mouseY);
        if (ent) {
          ent.emit('mousedown');
        }
      }
      ...
      for (let i = 0; i < this._entities.length; ++i) {
        ...
        if (parent === screenRoot) {
          ...
          width = parentWidget._calcWidth;
          height = parentWidget._calcHeight;
        }
      ...
      utils.walk(screenRoot, entity => {
        ...
        if (widgetComp) {
          width = widgetComp._calcWidth;
          height = widgetComp._calcHeight;
