...
export default class SkinningModelComponent extends Component {
  constructor() {
    ...
    this._model = new renderer.SkinningModel();
  }
  ...
  onEnable() {
    this._entity.emit('skinning-model-changed', this, 'enabled', true);
  }

  onDisable() {
    this._entity.emit('skinning-model-changed', this, 'enabled', false);
  }
  ...
  set mesh(val) {
    if (this._mesh !== val) {
      ...
      if (this._mesh.skinning) {
        let texture = utils.createJointsTexture(
          this._engine,
          this._mesh.skinning
        );
