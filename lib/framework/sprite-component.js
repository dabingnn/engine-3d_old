...
function _generateSlicedSpriteVerts(out, sprite, width, height) {
  ...
  // x0, x1, x2, x3
  let xs = new Float32Array(4);
  let ys = new Float32Array(4);
  let xScale = 1.0;
  let yScale = 1.0;
  ...
  xs[0] = 0;
  xs[1] = (sprite ? sprite._left : 0) * xScale;
  xs[2] = width - (sprite ? sprite._right : 0) * xScale;
  xs[3] = width;
  ys[0] = 0;
  ys[1] = (sprite ? sprite._bottom : 0) * yScale;
  ys[2] = height - (sprite ? sprite._top : 0) * yScale;
  ys[3] = height;
  for (let row = 0; row < 4; ++row) {
    for (let column = 0; column < 4; ++column) {
      vec3.set(out.positions[row * 4 + column], xs[column], ys[row], 0);
    }
  }
