...
export default class WidgetSystem extends System {
  ...
  tick() {
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      // process layout
      this._processLayout(_entities, screen);
    }
  ...
  _processLayout(entities, screen) {
    for (let i = 0; i < entities.length; ++i) {
      ...
      if (parentWidget === null) {
        ...
        parentX = 0.0;
        parentY = 0.0;
        parentWidth = this._app._canvas.width / screen.scaleFactor;
        parentHeight = this._app._canvas.height / screen.scaleFactor;
