...
export default class WidgetSystem extends System {
    ...
    if (input.hasTouch) {
      for (let i = 0; i < input.touchCount; ++i) {
        ...
        if (touchInfo.phase === 'start') {
          ...
          // NOTE: only first element can change focus
          if (i === 0) {
    ...
    if (mousemoved) {
      ...
      if (lastHovering !== targetEnt) {
        ...
        if (lastHovering) {
          ...
          mouseEvent.mouseX = input.mouseX;
          mouseEvent.mouseY = input.mouseY;
          mouseEvent.target = lastHovering;
          mouseEvent.button = 0;
          mouseEvent.buttons = input.mouseButtons;

          lastHovering.emit('mouseleave', mouseEvent);
        }
  ...
  _focus(ent) {
    if (this._focusedEntity === ent) {
      return;
    }

    let lastFocused = this._focusedEntity;
    this._focusedEntity = ent;
    ...
    if (ent) {
      ...
      focusEvent.target = ent;
      focusEvent.relatedTarget = lastFocused;

      ent.dispatch(focusEvent);
    }
  }

  _hover(ent) {
    if (this._hoveringEntity === ent) {
      return;
    }

    const input = this._app.input;
    let lastHovering = this._hoveringEntity;
    this._hoveringEntity = ent;
  ...
  _processLayout(entities) {
    for (let i = 0; i < entities.length; ++i) {
      ...
      if (parentWidget === null) {
        ...
        parentX = 0.0;
        parentY = 0.0;
        parentWidth = this._app._canvas.width;
        parentHeight = this._app._canvas.height;

        widget.setOffset(0, 0);
        widget.setAnchors(0, 0, 1, 1);
