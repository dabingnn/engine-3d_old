...
import { vec3, quat, color4 } from '../../vmath';
...
export default class ScrollBarComponent extends  UIElementComponent{
  constructor() {
    ...
    this._startPos = vec3.zero();
    this._offsetValue = 0.0;
    this._fingerId = -1;
    this._ratio = 0.0;
    this._calRectPos = vec3.zero();
  ...
  _updateValue(movePos) {
    ...
    let deltaValue = 0.0;
    let value = 0.0;
    let dirOffset = vec3.zero();;
    let mouse = vec3.new(movePos.x, movePos.y, 0);
    this._calRectPos = this._getWposRect();
    let anchorStart = this._direction === 'horizontal' ? this._handle.anchorLeft : this._handle.anchorBottom;
    let rect = this._getCalculateWidget()._rect;
  ...
  _clkBar(movePos) {
    ...
    let sqrt = 0.0;
    let a = vec3.zero();
    let length = this._direction === 'horizontal' ? this._getCalculateWidget()._rect.w : this._getCalculateWidget()._rect.h;
    this._calRectPos = this._getWposRect();
  ...
  _getRatio() {
    ...
    let calculate = this._getCalculateWidget();
    if (!calculate) {
      return vec3.zero();
    }

    calculate.entity.getWorldRot(wrot);
    let ratio = { x: 0, y: 0 };
  ...
  _getWposRect() {
    ...
    calculate.entity.getWorldPos(wpos);
    let rect = calculate._rect;
    let realX = 0.0, realY = 0.0;
    // get world rect pos
    if (this._direction === 'horizontal') {
      realX = wpos.x - (rect.w * calculate.pivotX * this._ratio.x) + (rect.h * calculate.pivotY * this._ratio.y);
      realY = wpos.y - (rect.w * calculate.pivotX * this._ratio.y) - (rect.h * calculate.pivotY * this._ratio.x);
    } else {
      realX = wpos.x + (rect.h * calculate.pivotY * -this._ratio.x) - (rect.w * calculate.pivotX * this._ratio.y);
      realY = wpos.y - (rect.h * calculate.pivotY * this._ratio.y) - (rect.w * calculate.pivotX * -this._ratio.x);
    }
  ...
  _getCalculateWidget() {
    if (!this._handle) {
      console.warn('There is no handle');
      return null;
    }

    let container = this._handle._entity.parent.getComp('Widget');
    return container;
  }
