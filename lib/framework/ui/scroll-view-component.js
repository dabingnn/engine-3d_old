...
import { vec2, vec3, mat4 } from '../../vmath';
...
export default class ScrollViewComponent extends UIElementComponent {
  constructor() {
    ...
    this._startOffset = vec3.zero();
    this._outOffset = vec3.zero();
    this._fingerId = -1;
  ...
  _lateUpdate() {
    this._getOutOffset(vec3.zero());
    if (!this._dragging) {
      let v = { x: this._outOffset.x, y: this._outOffset.y, z: 0 };
      if (!vec3.equals(this._outOffset, vec3.zero())) {
        let calOffset = this._getCalculateOffset();
        if (v.x !== 0) {
          if (this._movementType === 'elastic') {
