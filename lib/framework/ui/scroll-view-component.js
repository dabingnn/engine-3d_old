...
export default class ScrollViewComponent extends UIElementComponent {
  constructor() {
    ...
    this._velocity = vec2.create();
    this._startPos = vec2.create();
    this._startPoint = vec2.create();
    this._fingerId = -1;
    this._correctInited = false;
    this._prevContentBound = null;
    this._prevViewBound = null;
    this._prevPosition = vec2.create();
  ...
  _correctVisual() {
    this._getBound();
    if (
      this._viewBound && !vec3.equals(this._viewBound.size, vec3.create()) &&
      this._contentBound && !vec3.equals(this._contentBound.size, vec3.create())
    ) {
  ...
  _onDrag(mouse) {
    ...
    this._updateBound();
    let delta = vec2.create(), currentPos = vec2.create();
  ...
  _getBound() {
    ...
    // get parent bound
    let pCorners = [vec3.create(), vec3.create(), vec3.create(), vec3.create()];
  ...
  _updateBound() {
    ...
    if (this._movementType === 'clamp') {
      ...
      if (!vec2.equals(vec2.new(zero.x, zero.y), vec2.create())) {
        center = [];
  ...
  _lateUpdate() {
    ...
    this._getBound();
    let outOffset = this._calculateOffset(vec2.create());
    let isOut = !vec2.equals(outOffset, vec2.create());
    let isAdd = !vec2.equals(this._velocity, vec2.create());
    if (!this._dragging && (isOut || isAdd)) {
      ...
      if (!vec2.equals(this._velocity, vec2.create())) {
      ...
      } else if (!vec2.equals(outOffset, vec2.create())) {
