...
function _genSysFontVertsAndUpdateTexture(
  out,
  text,
  font,
  alignH,
  ...
  // apply font style
  let fontItalic = font._italic ? 'italic' : '';
  let fontBold = font._bold ? 'bold' : '';
  let fontStyle = `${fontItalic} ${fontBold} ${font._size}px ${font._fontFamily}`;
  fontCtx.font = fontStyle;

  // HACK: no good idea to figure out glyph's ascend.
  let startY = lineHeight * font._lineHeight * -0.15;
  let cursorY = startY;
  ...
  if (wrap) {
    for (let i = 0; i < paragraphs.length; ++i) {
      ...
      if (wrapInfos.length === 0) { // should not ignore empty line
        cursorY += lineHeight * font._lineHeight;
        drawCmd.push({
          text: '',
          start: 0,
          end: 0,
  ...
  for (let i = 0; i < drawCmd.length; ++i) {
    drawCmd[i].y += (h - textH) * alignV;
    _fillLine(drawCmd[i]);
  }

  font.texture.setImages([fontCanvas]);
  font.texture.commit();
}
...
export default class TextComponent extends Component {
  ...
  set font(val) {
    if (val !== this._font) {
      ...
      } else if (this._font.type === 'system') {
        // system font size should be same as text font size
        this._font._size = this._fontSize;
      }
      this._vertexDataDirty = true;
    }
  }
  ...
  set text(val) {
    ...
    if (this._font && this._font.type === 'opentype') {
      this._font.addText(this._text);
    } else if(this._font && this._font.type === 'system') {
  ...
  get fontFamily() {
    return this._font._fontFamily;
  }
  set fontFamily(val) {
    if (this._font._fontFamily !== val) {
      this._font._fontFamily = val;
      this._vertexDataDirty = true;
    }
  }

  get italic() {
    return this._font._italic;
  }
  set italic(val) {
    if (this._font._italic !== val) {
      this._font._italic = val;
      this._vertexDataDirty = true;
    }
  }

  get bold() {
    return this._font._bold;
  }
  set bold(val) {
    if (this._font._bold !== val) {
      this._font._bold = val;
      this._vertexDataDirty = true;
    }
  }
  ...
  set fontSize(val) {
    if (this._fontSize !== val) {
      this._fontSize = val;
      if (this._font && this._font.type === 'system') {
        // system font size should be same as text font size
        this._font._size = val;
      }
      this._vertexDataDirty = true;
    }
  }
  ...
  calcVertexData(x, y, w, h) {
    if (this._vertexDataDirty) {
      ...
      if (this._font && this._font.type === 'system') {
        _genSysFontVertsAndUpdateTexture(
