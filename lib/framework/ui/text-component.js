...
function _genSysFontVerts(out, texture) {
  vec3.set(out.lposList[0], 0, 0, 0);
  vec3.set(out.lposList[1], texture.width, 0, 0);
  vec3.set(out.lposList[2], 0, texture.height, 0);
  vec3.set(out.lposList[3], texture.width, texture.height, 0);
...
export default class TextComponent extends Component {
  ...
  set font(val) {
    if (val !== this._font) {
      ...
      } else if(this._font.type === 'system' && this._text !== '') {
        this._font.text = this._text;
      }
      this._vertexDataDirty = true;
    }
  }
  ...
  calcVertexData(x, y, w, h) {
    if (this._vertexDataDirty) {
      ...
      if (this._font && this._font.type === 'system') {
        _genSysFontVerts(this._cachedVertexData, this._font.texture);
      } else {
        _genFontVerts(
