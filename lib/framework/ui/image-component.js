...
function _genSimpleVerts(out, sprite, x, y, w, h) {
  out.lposList[0].set( x, y, 0);
  out.lposList[1].set( x + w, y, 0);
  out.lposList[2].set( x, y + h, 0);
  out.lposList[3].set( x + w, y + h, 0);
...
function _genSlicedVerts(out, sprite, x, y, w, h) {
  ...
  for (let row = 0; row < 4; ++row) {
    for (let column = 0; column < 4; ++column) {
      out.lposList[row * 4 + column].set( _x_tmp[column], _y_tmp[row], 0.0);
    }
  }
...
function _genFilledHorizontalVerts(out, sprite, x, y, w, h, start, range) {
  ...
  out.lposList[0].set( w * start + x, y, 0);
  out.lposList[1].set( w * end + x, y, 0);
  out.lposList[2].set( w * start + x, y + h, 0);
  out.lposList[3].set( w * end + x, y + h, 0);
...
function _genFilledVerticalVerts(out, sprite, x, y, w, h, start, range) {
  ...
  out.lposList[0].set( x, y + h * start, 0);
  out.lposList[1].set( w + x, y + h * start, 0);
  out.lposList[2].set( x, y + h * end, 0);
  out.lposList[3].set( w + x, y + h * end, 0);
