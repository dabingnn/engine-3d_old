...
function _reallocVertexData(type, filledType) {
  let vertexCount = 4;
  let indices = null;
...
function _genFilledHorizontalVerts(out, sprite, x, y, w, h, start, range) {
  ...
  let uv = vec3.create();
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[3], start);
  vec3.copy(out.uvs[0], uv);
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[3], end);
  vec3.copy(out.uvs[1], uv);
  vec3.lerp(uv, sprite._uvs[12], sprite._uvs[15], start);
  vec3.copy(out.uvs[2], uv);
  vec3.lerp(uv, sprite._uvs[12], sprite._uvs[15], end);
  vec3.copy(out.uvs[3], uv);
...
function _genFilledVerticalVerts(out, sprite, x, y, w, h, start, range) {
  ...
  let uv = vec3.create();
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[12], start);
  vec3.copy(out.uvs[0], uv);
  vec3.lerp(uv, sprite._uvs[3], sprite._uvs[15], start);
  vec3.copy(out.uvs[1], uv);
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[12], end);
  vec3.copy(out.uvs[2], uv);
  vec3.lerp(uv, sprite._uvs[3], sprite._uvs[15], end);
  vec3.copy(out.uvs[3], uv);
...
// todo: this is a hack version for radial filled sprite
let _genFilledRadialVerts = (() => {

  let intersectionPoints_1 = [vec3.create(), vec3.create(), vec3.create(), vec3.create()];
  let intersectionPoints_2 = [vec3.create(), vec3.create(), vec3.create(), vec3.create()];
  let intersectionUVs_1 = [vec3.create(), vec3.create(), vec3.create(), vec3.create()];
  let intersectionUVs_2 = [vec3.create(), vec3.create(), vec3.create(), vec3.create()];
  ...
  return (out, sprite, x, y, w, h, start, range, cx, cy) => {
    ...
    quadUVs.length = 0;
    quadUVs.push(sprite.uvs[0], sprite.uvs[3], sprite.uvs[15], sprite.uvs[12]);
    //fast return ,generate simple mesh if range is bigger than 1
    // if (fillrange >= 1) return this.generateSimpleMesh(size, anchor);

    //clamp range, center and start
    range = range < 0 ? 0 : range;
    while (start >= 1) start = start - 1;
    ...
    _getInsectedPoints(rawQuad, center, start, intersectionPoints_1, intersectionUVs_1);
    _getInsectedPoints(rawQuad, center, end, intersectionPoints_2, intersectionUVs_2);
    //iterate each edge
      ...
      for (let testIndex = 0; testIndex < 3; ++testIndex) {
        if (startAngle >= end) {
          ...
          if (endAngle >= end) {
            ...
            vec3.copy(out.lposList[vertCount + 1], intersectionPoints_2[index]);

            vec3.copy(out.uvs[vertCount], quadUVs[index]);
            vec3.copy(out.uvs[vertCount + 1], intersectionUVs_2[index]);
          ...
          if (endAngle <= start) {
            ...
            vec3.copy(out.lposList[vertCount], intersectionPoints_1[index]);
            vec3.copy(out.lposList[vertCount + 1], quadVertPoses[(index + 1) % 4]);

            vec3.copy(out.uvs[vertCount], intersectionUVs_1[index]);
            vec3.copy(out.uvs[vertCount + 1], quadUVs[(index + 1) % 4]);
...
export default class ImageComponent extends Component {
  constructor() {
    ...
    // only available for filled sprite
    this._filledType = 'horizontal';
    this._fillStart = 0.2;
    this._fillRange = 0.8;

    // only available for filled radial sprite
    this._filledCenter = vec2.new(0.5, 0.5);
  }
  ...
  set type(val) {
    if (val !== this._type) {
      ...
      this._cachedVertexData = _reallocVertexData(this._type, this._filledType);
      this._vertexDataDirty = true;
    }
  }

  set filledType(val) {
    if (val !== this._filledType) {
      this._filledType = val;
  ...
  get filledCenter() {
    return this._filledCenter;
  }

  set filledCenter(val) {
    vec2.copy(this._filledCenter, val);
    if (this._type === 'filled' && this._filledType === 'radial') {
      this._vertexDataDirty = true;
    }
  }
  ...
  calcVertexData(x, y, w, h) {
    if (this._vertexDataDirty) {
      ...
      if (this._type === 'simple') {
        ...
        if (this._filledType === 'radial') {
          // radial
          if (this._fillRange >= 1) {
            _genSimpleVerts(this._cachedVertexData, sprite, x, y, w, h);
            this._cachedVertexData.indices = _simpleIndices;
          } else {
            _genFilledRadialVerts(this._cachedVertexData, sprite, x, y, w, h, this._fillStart, this._fillRange, this._filledCenter.x, this._filledCenter.y);
            this._cachedVertexData.indices = _fullRadialIndices;
          }
        } else if (this._filledType === 'vertical') {
