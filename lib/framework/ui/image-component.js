...
function _genFilledRadialVerts(out, sprite, x, y, w, h, start, range) {
  start = start < 0 ? 0 : start;
  start = start > 1 ? 1 : start;
  range = range < 0 ? 0 : range;
  let end = (range + start > 1) ? 1 : (range + start);
  ...
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[12], start);
  vec3.copy(out.uvs[0], uv);
  vec3.lerp(uv, sprite._uvs[1], sprite._uvs[15], start);
  vec3.copy(out.uvs[1], uv);
  vec3.lerp(uv, sprite._uvs[0], sprite._uvs[12], end);
  vec3.copy(out.uvs[2], uv);
  vec3.lerp(uv, sprite._uvs[1], sprite._uvs[15], end);
  vec3.copy(out.uvs[3], uv);
    ...
    if (this._vertexDataDirty) {
      ...
      if (this._type === 'simple') {
        ...
        if (this._filledType === 'radial') {
          // radial
          _genFilledRadialVerts(this._cachedVertexData, sprite, x, y, w, h, this._fillStart, this._fillRange);
