...
export default class TextureAnimationModule {
  ...
  animate(p) {
    ...
    let startFrame = this._startFrame.evaluate(normalizedTime) / (this._numTilesX * this._numTilesY);
    if (this._animation == 'wholeSheet') {
      p.frameIndex = repeat(this._cycleCount * (this._frameOverTime.evaluate(normalizedTime) + startFrame), 1);
    }
    else if (this._animation == 'singleRow') {
      let rowLength = 1 / this._numTilesY;
      if (this._randomRow) {
        let f = repeat(this._cycleCount * (this._frameOverTime.evaluate(normalizedTime) + startFrame), 1);
        ...
        let to = from + rowLength;
        p.frameIndex = lerp(from, to, repeat(this._cycleCount * (this._frameOverTime.evaluate(normalizedTime) + startFrame), 1));
      }
    }
  }
}
