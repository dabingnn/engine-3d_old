...
export default class ParticleSystemComponent extends Component {
  ...
  emit(count, emitParams = null) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      particle.rotation.set(this._startRotation.evaluate(this._time/this._duration),0,0);

      // apply startSize. now 2D only.
      particle.startSize.set(this._startSize.evaluate(this._time/this._duration),0,0);
  ...
  setCustomData1(x, y) {
    this._customData1.set( x, y);
  }

  setCustomData2(x, y) {
    this._customData2.set( x, y);
  }
