...
export default class ParticleSystemComponent extends Component {
  ...
  emit(count, emitParams = null) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      vec3.scale(particle.velocity, this._shape.generateEmitDirection(), this._startSpeed);
      // particle.velocity = vec3.new(5, 0, 0);
      particle.startSize = vec3.new(randomRange(0.5, 2), randomRange(0.5, 2), randomRange(0.5, 2));
...
ParticleSystemComponent.schema = {
  ...
  startColor: { // TODO: color, gradient, random between two colors, random between two gradients, random color
    type: 'color3',
    default: [1, 1, 1],
    set(val) {
  ...
  startSize: { // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 1,
    set(val) {
      this._startSize = val;
  ...
  startSpeed: { // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 5,
    set(val) {
  ...
  startRotation: { // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 0,
    set(val) {
  ...
  startLifetime: { // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 5,
    set(val) {
  ...
  rateOverDistance: { // // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 10,
    set(val) {
  ...
  gravityModifier: { // TODO: constant, curve, random between two constants, random between two curves
    type: 'number',
    default: 0,
    set(val) {
    ...
    set(val) {
      ...
      switch(val) {
        case 'cone':
          ...
          break;
        case 'box':
          this._shape = this._app.createObject('BoxShape');// new BoxShape();
          break;
        case 'sphere':
          // TODO:
          break;
