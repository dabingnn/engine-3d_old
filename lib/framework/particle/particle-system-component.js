...
export default class ParticleSystemComponent extends Component {
  ...
  emit(count, emitParams = null) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      switch(this._startLifetimeType) {
        case 'constant':
          particle.startLifetime = this._startLifetime;
        ...
        case 'curve':
          ...
          break;
        case 'randomBetweenTwoConstants':
          particle.startLifetime = randomRange(this._startLifetimeMin, this._startLifetimeMax);
  ...
  _updateParticles(dt) {
    for (let i = 0; i < this._particles.length; ++i) {
      ...
      p.velocity.y -= this._gravityModifier * 9.8 * dt; // apply gravity.
    ...
    for (let i = 0; i < this._particles.length; ++i) {
      ...
      for (let j = 0; j < 4; ++j) { // four verts per particle.
        ...
        if (this._vertAttrFlags.color) {
          if (this._enableColorOverLifetime) {
            let gradientColor = this._getColorByLifetime(p);
            attrs.push(color4.set(_vertAttrsCache.color,
              p.startColor.r * gradientColor.r,
              p.startColor.g * gradientColor.g,
              p.startColor.b * gradientColor.b,
              p.startColor.a * gradientColor.a));
          } else {
  ...
  _getColorByLifetime(particle) {
    let time = 1.0 - particle.remainingLifetime / particle.startLifetime;
    if (this._colorOverLifetimeGradient.mode === 'gradient') {
      return this._colorOverLifetimeGradient.gradient.getRGBA(time);
    } else {
      console.warn('now colorOverLifetime only support gradient mode');
    }
  }
}
