...
import BoxShape from './emitter/box-shape';
import CircleShape from './emitter/circle-shape';
import EdgeShape from './emitter/edge-shape';
import SphereShape from './emitter/sphere-shape';
import HemisphereShape from './emitter/hemisphere-shape';
import DefaultEmitter from './emitter/default-emitter';
import { vec3, color4, vec2, mat4, quat } from '../../vmath';
import Schema from '../../ecs/schema';
...
registry.registerClass("BoxShape",BoxShape);
registry.registerClass("CircleShape",CircleShape);
registry.registerClass("EdgeShape",EdgeShape);
...
export default class ParticleSystemComponent extends Component {
  ...
  emit(count, emitParams = null) {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      // apply position.
      vec3.copy(particle.position, this._shapeModule.generateEmitPosition());
      // apply startSpeed.
      vec3.scale(particle.velocity,this._shapeModule.generateEmitDirection(),this._startSpeed.evaluate(this._time/this._duration));
  ...
  _updateParticles(dt) {
    for (let i = 0; i < this._particles.length; ++i) {
      ...
      vec3.add(this.tempVel,p.velocity,p.animatedVelocity);
      if(this._velocityOvertimeModule.enable) {
        vec3.scale(this.tempVel,this.tempVel,this._velocityOvertimeModule._speedModifier.evaluate(1-p.remainingLifetime/p.startLifetime));
      }
      vec3.scaleAndAdd(p.position, p.position, this.tempVel, dt); // apply velocity.
    }
  }
  ...
  set shape(val) {
    this._shapeModule = createShapeModule(val);
  }
}
...
ParticleSystemComponent.schema = {
  ...
  shapeModule:{
    type:"Object",
    default:null,
    parse:function (app, data, propInfo, entities){
      if(data === null) {
        return new DefaultEmitter();
      }
      //if data is not a JSON object,just return the data
      if(Object.getPrototypeOf(Object.getPrototypeOf(data)) !== null)
        return data;
      let shape = createShapeModule(data.type);
      Schema.parse(app,shape,data,entities);
      return shape;
    }
  },
