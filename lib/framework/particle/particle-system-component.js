// Copyright (c) 2017-2018 Xiamen Yaji Software Co., Ltd.

import { Component } from 'ecs.js';
import renderer from 'renderer.js';
import Material from '../../assets/material';
import gfx from 'gfx.js';
import { RecyclePool } from 'memop';
import Particle from './particle';
import { vec3, random, randomRange, color4, vec2, quat } from 'vmath';

let _vertAttrsCache = {
  position: vec3.create(),
  uv: vec2.create(),
  uv0: vec2.create(),
  color: color4.create(),
  normal: vec3.create(),
  tangent: vec3.create(),
};

let _name2VertAttrs = {
  'Position': { name: gfx.ATTR_POSITION, type: gfx.ATTR_TYPE_FLOAT32, num: 3, flag: 'position' },
  'UV': { name: gfx.ATTR_UV, type: gfx.ATTR_TYPE_FLOAT32, num: 2, flag: 'uv' },
  'UV0': { name: gfx.ATTR_UV0, type: gfx.ATTR_TYPE_FLOAT32, num: 2, flag: 'uv0' }, // size, unused
  'Color': { name: gfx.ATTR_COLOR, type: gfx.ATTR_TYPE_FLOAT32, num: 4, flag: 'color' },
  'Normal': { name: gfx.ATTR_NORMAL, type: gfx.ATTR_TYPE_FLOAT32, num: 3, flag: 'normal' }, // 3D only
  'Tangent': { name: gfx.ATTR_TANGENT, type: gfx.ATTR_TYPE_FLOAT32, num: 3, flag: 'tangent' }, // 3D only
  'Custom1': {name: gfx.ATTR_UV1, type: gfx.ATTR_TYPE_FLOAT32, num: 2, flag: 'custom1' },
  'Custom2': {name: gfx.ATTR_UV2, type: gfx.ATTR_TYPE_FLOAT32, num: 2, flag: 'custom2' },
};

let _uvs = [
  0, 0, // bottom-left
  1, 0, // bottom-right
  0, 1, // top-left
  1, 1  // top-right
];

export default class ParticleSystemComponent extends Component {
  constructor() {
    super();

    this._model = null;
    this._shape = null;

    // internal status
    this._isPlaying = false;
    this._isPaused = false;
    this._isStopped = true;
    this._isEmitting = false;

    this._time = 0.0;  // playback position in seconds.
    this._emitRateCounter = 0.0;

    this._customData1 = vec2.create();
    this._customData2 = vec2.create();

    this._subEmitters = []; // array of { emitter: ParticleSystemComponent, type: 'birth', 'collision' or 'death'}
    this._vertAttrs = [];
    this._vertAttrFlags = {
      position: true,
      uv: true,
      uv0: true,
      color: true,
      normal: false,
      tangent: false,
      custom1: false,
      custom2: false,
    };

  }

  onInit() {
    this._particles = new RecyclePool(() => {
      return new Particle(this);
    }, this._capacity);
    this._model = new renderer.ParticleBatchModel(this._app.device, this._capacity);
    this._model.setNode(this._entity);
    this._shape = this._app.createObject('BoxShape');
    // HACK, TODO
    if (this._material === null) {
      this._material = new Material();
      this._material.effect = this._app.assets.get('builtin-effect-particle-premultiply-blend');
    }

    // apply startDelay.
    if (this._startDelayType === 'randomBetweenTwoConstants') {
      this._startDelay = randomRange(this._startDelayMin, this._startDelayMax);
    }

    // apply rateOverTime
    // TODO: other types.
    if (this._rateOverTimeType === 'randomBetweenTwoConstants') {
      this._rateOverTime = randomRange(this._rateOverTimeMin, this._rateOverTimeMax);
    }

    // apply gravityModifier.
    // TODO: other types.
    if (this._gravityModifierType === 'randomBetweenTwoConstants') {
      this._gravityModifier = randomRange(this._gravityModifierMin, this._gravityModifierMax);
    }

    this._system.add(this);
  }

  onDestroy() {
    this._system.remove(this);
  }

  onEnable() {
    this._app.scene.addModel(this._model);
  }

  onDisable() {
    this._app.scene.removeModel(this._model);
  }

  set material(val) {
    this._material = val;
    this._model.setEffect(val.effectInst);
  }

  setVertexAtrributes(attrs) {
    // clear vertex attribute flags
    for (let key in this._vertAttrFlags) {
      this._vertAttrFlags[key] = false;
    }

    for (let i = 0; i < attrs.length; ++i) {
      let attr = _name2VertAttrs(attrs[i]);
      if (attr !== undefined) {
        this._vertAttrs.push(attr);
        this._vertAttrFlags[attr.flag] = true;
      } else {
        console.error('vertex attribute name wrong.');
      }
    }
    this._model.setVertexAttributes(this._vertAttrs);
  }

  // TODO: fastforward current particle system by simulating particles over given period of time, then pause it.
  // simulate(time, withChildren, restart, fixedTimeStep) {

  // }

  play() {
    if (this._isPaused) {
      this._isPaused = false;
    }
    if (this._isStopped) {
      // this._app.scene.addModel(this._model);
      this._isStopped = false;
    }

    this._isPlaying = true;
  }

  pause() {
    if (this._isStopped) {
      console.warn('pause(): particle system is already stopped.');
      return;
    }
    if (this._isPlaying) {
      this._isPlaying = false;
    }

    this._isPaused = true;
  }

  stop() {
    if (this._isPlaying) {
      this._isPlaying = false;
    }
    if (this._isPaused) {
      this._isPaused = false;
    }

    this.clear();
    this._time = 0.0;
    this._emitRateCounter = 0.0;
    // this._app.scene.removeModel(this._model);

    this._isStopped = true;
  }

  // remove all particles from current particle system.
  clear() {
    this._particles.reset();
  }

  emit(count, emitParams = null) {
    if (emitParams !== null) {
      // TODO:
    }

    for (let i = 0; i < count; ++i) {
      if (this._particles.length >= this._capacity) {
        return;
      }

      let particle = this._particles.add();

      // apply position.
      vec3.copy(particle.position, this._shape.generateEmitPosition());

      // subEmitter
      // if (this._subEmitters.length > 0) {
      //   for (let idx = 0; idx < this._subEmitters.length; ++idx) {
      //     let subEmitter = this._subEmitters[idx];
      //     if (subEmitter.type === 'birth') {
      //       // TODO: clone subEmitter
      //       vec3.copy(subEmitter.emitter.entity.lpos, particle.position);
      //       subEmitter.emitter.play();
      //     }
      //   }
      // }

      // apply startSpeed.
      switch(this._startSpeedType) {
        case 'constant':
          vec3.scale(particle.velocity,
            this._shape.generateEmitDirection(),
            this._startSpeedConst);
          break;
        case 'curve':
          // TODO:
          break;
        case 'randomBetweenTwoConstants':
          vec3.scale(particle.velocity,
            this._shape.generateEmitDirection(),
            randomRange(this._startSpeedConstMin, this._startSpeedConstMax));
            break;
        case 'randomBetweenTwoCurves':
          // TODO:
          break;
      }

      // apply startSize. now 2D only.
      switch(this._startSizeType) {
        case 'constant':
          vec3.set(particle.startSize,
            this._startSizeConst,
            0,
            0);
          break;
        case 'curve':
          // TODO:
          break;
        case 'randomBetweenTwoConstants':
          vec3.set(particle.startSize,
            randomRange(this._startSizeConstMin, this._startSizeConstMax),
            0,
            0);
          break;
        case 'randomBetweenTwoCurves':
          // TODO:
          break;
      }

      // apply startColor.
      switch(this._startColorType) {
        case 'color':
          color4.copy(particle.startColor, this._startColor);
          break;
        case 'gradient':
          // TODO:
          break;
        case 'randomBetweenTwoColors':
          // color4.lerp or randomize individual rgba ?
          color4.set(particle.startColor,
            randomRange(this._startColor1.r, this._startColor2.r),
            randomRange(this._startColor1.g, this._startColor2.g),
            randomRange(this._startColor1.b, this._startColor2.b),
            randomRange(this._startColor1.a, this._startColor2.a));
          break;
        case 'randomBetweenTwoGradients':
          // TODO:
          break;
        case 'randomColor':
          color4.set(particle.startColor,
            random(),
            random(),
            random(),
            random()
          );
          break;
      }

      // apply startLifetime.
      switch(this._startLifetimeType) {
        case 'constant':
          particle.remainingLifetime = this._startLifetimeConst;
          break;
        case 'curve':
          // TODO:
          break;
        case 'randomBetweenTwoConstants':
          particle.remainingLifetime = randomRange(this._startLifetimeConstMin, this._startLifetimeConstMax);
          break;
        case 'randomBetweenTwoCurves':
          // TODO:
          break;
      }

    } // end of particles forLoop.
  }

  // simulation, update particles.
  _update(dt) {
    for (let i = 0; i < this._particles.length; ++i) {
      let p = this._particles.data[i];
      p.remainingLifetime -= dt;

      if(p.remainingLifetime < 0.0) {
        // subEmitter
        // if (this._subEmitters.length > 0) {
        //   for (let idx = 0; idx < this._subEmitters.length; ++idx) {
        //     let subEmitter = this._subEmitters[idx];
        //     if (subEmitter.type === 'death') {
        //       vec3.copy(subEmitter.emitter.entity.lpos, p.position);
        //       subEmitter.emitter.play();
        //     }
        //   }
        // }

        this._particles.remove(i);
        --i;
        continue;
      }

      p.velocity.y -= this._gravityModifier * dt; // apply gravity.
      vec3.scaleAndAdd(p.position, p.position, p.velocity, dt); // apply velocity.
    }
  }

  tick(dt) {
    // console.info(`dt = ${dt}`);
    let scaledDeltaTime = dt * this._simulationSpeed;
    if (this._isPlaying) {
      this._time += scaledDeltaTime;

      // emit particles.
      if (this._time > this._startDelay) {
        if (!this._isStopped) {
          this._isEmitting = true;
        }
        if (this._time > (this._duration + this._startDelay)) {
          this._time = this._startDelay; // delay will not be applied from the second loop.(Unity)
          this._emitRateCounter = 0.0;
          if (!this._loop) {
            this._isEmitting = false;
            this._isStopped = true;
          }
        }
        this._emitRateCounter += this._rateOverTime * scaledDeltaTime;
        if (this._emitRateCounter > 1 && this._isEmitting) {
          let emitNum = Math.floor(this._emitRateCounter);
          this._emitRateCounter -= emitNum;
          this.emit(emitNum);
        }
      }

      // simulation, update particles.
      this._update(scaledDeltaTime);

      // update vertex buffer
      let idx = 0;
      for (let i = 0; i < this._particles.length; ++i) {
        let p = this._particles.data[i];
        for (let j = 0; j < 4; ++j) { // four verts per particle.
          let attrs = [];
          if (this._vertAttrFlags.position) {
            attrs.push(vec3.set(_vertAttrsCache.position, p.position.x, p.position.y, p.position.z));
          }
          if (this._vertAttrFlags.uv) {
            attrs.push(vec2.set(_vertAttrsCache.uv, _uvs[2 * j], _uvs[2 * j + 1]));
          }
          if (this._vertAttrFlags.uv0) {
            attrs.push(vec2.set(_vertAttrsCache.uv0, p.startSize.x , 0));
          }
          if (this._vertAttrFlags.color) {
            attrs.push(color4.set(_vertAttrsCache.color, p.startColor.r, p.startColor.g, p.startColor.b, p.startColor.a));
          }
          // TODO: other attrs.
          if (this._vertAttrFlags.custom1) {
            attrs.push(this._customData1);
          }
          if (this._vertAttrFlags.custom2) {
            attrs.push(this._customData2);
          }

          this._model.addParticleVertexData(idx++, attrs);
        }
      }

      // because we use index buffer, per particle index count = 6.
      this._model.updateIA(this._particles.length * 6);
    }
  }

  addSubEmitter(subEmitter) {
    this._subEmitters.push(subEmitter);
  }

  removeSubEmitter(idx) {
    this._subEmitters.remove(idx);
  }

  getParticleCount() {
    return this._particles.length;
  }

  setCustomData1(val) {
    if (val instanceof vec2) {
      vec2.copy(this._customData1, val);
    } else {
      console.warn('CustomData1 only support vec2 type.');
    }
  }

  setCustomData2(val) {
    if (val instanceof vec2) {
      vec2.copy(this._customData2, val);
    } else {
      console.warn('CustomData2 only support vec2 type.');
    }
  }

  get isPlaying() {
    return this._isPlaying;
  }

  get isPaused() {
    return this._isPaused;
  }

  get isStopped() {
    return this._isStopped;
  }

  get isEmitting() {
    return this._isEmitting;
  }

  get time() {
    return this._time;
  }
}

ParticleSystemComponent.schema = {
  material: {
    type: 'asset',
    default: null,
    set(val) {
      if (this._material === val) {
        return;
      }

      this._material = val;
      this._model.setEffect(val.effectInst);
    }
  },

  // main module properties
  capacity: {
    type: 'number',
    default: 2000,
  },

  startColorType: {
    type: 'enums',
    default: 'color',
    options: [
      'color',
      'gradient',
      'randomBetweenTwoColors',
      'randomBetweenTwoGradients',
      'randomColor'
    ],
    set(val) {
      this._startColorType = val;
    }
  },

  startColor: {
    type: 'color4',
    default: [1, 1, 1, 1],
    set(val) {
      this._startColor = val;
    }
  },

  startColor1: {
    type: 'color4',
    default: [1, 1, 1, 1],
    set(val) {
      this._startColor1 = val;
    }
  },

  startColor2: {
    type: 'color4',
    default: [1, 1, 1, 1],
    set(val) {
      this._startColor2 = val;
    }
  },

  // TODO: color gradient.

  startSizeType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._startSizeType = val;
    }
  },

  startSizeConst: {
    type: 'number',
    default: 1,
    set(val) {
      this._startSizeConst = val;
    }
  },

  startSizeConstMin: {
    type: 'number',
    default: 1,
    set(val) {
      this._startSizeConstMin = val;
    }
  },

  startSizeConstMax: {
    type: 'number',
    default: 1,
    set(val) {
      this._startSizeConstMax = val;
    }
  },

  // TODO: startSize curve.

  startSpeedType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._startSpeedType = val;
    }
  },

  startSpeedConst: {
    type: 'number',
    default: 5,
    set(val) {
      this._startSpeedConst = val;
    }
  },

  startSpeedConstMin: {
    type: 'number',
    default: 5,
    set(val) {
      this._startSpeedConstMin = val;
    }
  },

  startSpeedConstMax: {
    type: 'number',
    default: 5,
    set(val) {
      this._startSpeedConstMax = val;
    }
  },

  // TODO: startSpeed curve.

  startRotationType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._startRotationType = val;
    }
  },

  startRotationConst: {
    type: 'number',
    default: 0,
    set(val) {
      this._startRotationConst = val;
    }
  },

  startRotationConstMin: {
    type: 'number',
    default: 0,
    set(val) {
      this._startRotationConstMin = val;
    }
  },

  startRotationConstMax: {
    type: 'number',
    default: 60,
    set(val) {
      this._startRotationConstMax = val;
    }
  },

  // TODO: startRotation curve.

  startDelayType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'randomBetweenTwoConstants'
    ],
    set(val) {
      this._startDelayType = val;
    }
  },

  startDelay: {
    type: 'number',
    default: 0,
    set(val) {
      this._startDelay = val;
    }
  },

  startDelayMin: {
    type: 'number',
    default: 0,
    set(val) {
      this._startDelayMin = val;
    }
  },

  startDelayMax: {
    type: 'number',
    default: 0,
    set(val) {
      this._startDelayConstMax = val;
    }
  },

  startLifetimeType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randowmBetweenTwoCurves'
    ],
    set(val) {
      this._startLifetimeType = val;
    }
  },

  startLifetimeConst: {
    type: 'number',
    default: 5,
    set(val) {
      this._startLifetimeConst = val;
    }
  },

  startLifetimeConstMin: {
    type: 'number',
    default: 5,
    set(val) {
      this._startLifetimeConstMin = val;
    }
  },

  startLifetimeConstMax: {
    type: 'number',
    default: 5,
    set(val) {
      this._startLifetimeConstMax = val;
    }
  },

  // TODO: startLifetime curve.

  // can only be set when not playing.
  duration: {
    type: 'number',
    default: 5,
    set(val) {
      this._duration = val;
    }
  },

  loop: {
    type: 'boolean',
    default: true,
    set(val) {
      this._loop = val;
    }
  },

  simulationSpeed: {
    type: 'number',
    default: 1,
    set(val) {
      this._simulationSpeed = val;
    }
  },

  gravityModifierType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._gravityModifierType = val;
    }
  },

  gravityModifier: {
    type: 'number',
    default: 0,
    set(val) {
      this._gravityModifier = val;
    }
  },

  gravityModifierMin: {
    type: 'number',
    default: 0,
    set(val) {
      this._gravityModifierMin = val;
    }
  },

  gravityModifierMax: {
    type: 'number',
    default: 0,
    set(val) {
      this._gravityModifierMax = val;
    }
  },

  // TODO: gravityModifier curve.


  // emission module
  rateOverTimeType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._rateOverTimeType = val;
    }
  },

  rateOverTime: {
    type: 'number',
    default: 10,
    set(val) {
      this._rateOverTime = val;
    }
  },

  rateOverTimeMin: {
    type: 'number',
    default: 10,
    set(val) {
      this._rateOverTimeMin = val;
    }
  },

  rateOverTimeMax: {
    type: 'number',
    default: 10,
    set(val) {
      this._rateOverTimeMax = val;
    }
  },

  // TODO: rateOverTime curve.

  rateOverDistanceType: {
    type: 'enums',
    default: 'constant',
    options: [
      'constant',
      'curve',
      'randomBetweenTwoConstants',
      'randomBetweenTwoCurves'
    ],
    set(val) {
      this._rateOverDistanceType = val;
    }
  },

  rateOverDistance: {
    type: 'number',
    default: 0,
    set(val) {
      this._rateOverDistance = val;
    }
  },

  rateOverDistanceMin: {
    type: 'number',
    default: 0,
    set(val) {
      this._rateOverDistanceMin = val;
    }
  },

  rateOverDistanceMax: {
    type: 'number',
    default: 0,
    set(val) {
      this._rateOverDistanceMax = val;
    }
  },

  // TODO: rateOverDistance curve.


  // shape module
  shapeType: {
    type: 'enums',
    default: 'box',
    options: [
      'cone',
      'box',
      'sphere',
      'hemisphere',
      'circle',
      'edge',
      'mesh',
    ],
    set(val) {
      if (this._shapeType === val) {
        return;
      }

      switch(val) {
        case 'cone':
          // TODO:
          break;
        case 'box':
          this._shape = this._app.createObject('BoxShape');
          break;
        case 'sphere':
          this._shape = this._app.createObject('SphereShape');
          break;
        case 'hemisphere':
          this._shape = this._app.createObject('HemisphereShape');
          break;
        case 'circle':
          this._shape = this._app.createObject('CircleShape');
          break;
        case 'edge':
          this._shape = this._app.createObject('EdgeShape');
          break;
        case 'mesh':
          // TODO:
          break;
        default:
          console.warn(`emit shape ${val} not support.`);
      }

      this._shapeType = val;
    }
  },

};