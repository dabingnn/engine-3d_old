...
import { vec3, random, randomRange, color4, vec2, toRadian, mat4, quat } from '../../vmath';
    ...
    for (let i = 0; i < count; ++i) {
      ...
      switch(this._simulationSpace) {
        case 'local':
          ...
          break;
        case 'world': {
            let emitPos = this._shapeModule.generateEmitPosition();
  ...
  _updateParticles(dt) {
    for (let i = 0; i < this._particles.length; ++i) {
      ...
      if(this._colorOverLifetimeModule.enable) {
        this._colorOvertimeModule.animate(p);
      }
    ...
    parse:function (app, data, propInfo, entities){
      ...
      switch(data.type)
      {
        case "box":
          ...
          Schema.parse(app,box,data,entities);
          return box;
        case "circle":
          let circle = new CircleShape();
          Schema.parse(app,circle,data,entities)
          return circle;
        case "cone":
        case "sphere":
          let sphere = new SphereShape();
          Schema.parse(app,sphere,data,entities);
          return sphere;
        case "hemishpere":
          let hemiSphere = new HemisphereShape();
          Schema.parse(app,hemiSphere,data,entities);
          return hemiSphere;
        case "disable":
