    ...
    for (let i = 0; i < this.particleSystem._particles.length; ++i) {
      ...
      for (let j = 0; j < 4; ++j) { // four verts per particle.
        ...
        if (this._vertAttrFlags.color) {
          attrs.push(color4.set(_vertAttrsCache.color, p.color.r, p.color.g, p.color.b, p.color.a));
        }
        // TODO: other attrs.
        ...
        if (this._vertAttrFlags.color0) {
          ...
          let scale = this._velocityScale * vec3.length(this.tempVel) + this._lengthScale * p.size.x;
          attrs.push(color4.set(_vertAttrsCache.color0, this.tempVel.x, this.tempVel.y, this.tempVel.z, scale));
        }
