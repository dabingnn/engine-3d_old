    ...
    for (let i = 0; i < this.particleSystem._particles.length; ++i) {
      ...
      for (let j = 0; j < 4; ++j) { // four verts per particle.
        ...
        if (this._vertAttrFlags.color0) {
          let scale = this._velocityScale * vec3.length(p.ultimateVelocity) + this._lengthScale * p.size.x;
          attrs.push(color4.set(_vertAttrsCache.color0, p.ultimateVelocity.x, p.ultimateVelocity.y, p.ultimateVelocity.z, scale));
        }
