    ...
    for (let i = 0; i < this.particleSystem._particles.length; ++i) {
      ...
      for (let j = 0; j < 4; ++j) { // four verts per particle.
        ...
        if (this._vertAttrFlags.uv) {
          attrs.push(_vertAttrsCache.uv.set( _uvs[2 * j], _uvs[2 * j + 1]));
        }
        ...
        if (this._vertAttrFlags.color0) {
          let scale = this._velocityScale * p.ultimateVelocity.length() + this._lengthScale * p.size.x;
          attrs.push(_vertAttrsCache.color0.set( p.ultimateVelocity.x, p.ultimateVelocity.y, p.ultimateVelocity.z, scale));
        }
