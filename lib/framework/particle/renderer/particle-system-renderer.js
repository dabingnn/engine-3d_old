...
export default class ParticleSystemRenderer {
  ...
  _updateMaterialParams() {
    ...
    if (this.particleSystem.textureAnimationModule.enable) {
      this._material.setProperty('frameTile', this.frameTile.set( this.particleSystem.textureAnimationModule.numTilesX, this.particleSystem.textureAnimationModule.numTilesY));
    }
    else {
    ...
    for (let i = 0; i < this.particleSystem._particles.length; ++i) {
      ...
      for (let j = 0; j < 4; ++j) { // four verts per particle.
        ...
        if (this._vertAttrFlags.position) {
          attrs.push(_vertAttrsCache.position.set( p.position.x, p.position.y, p.position.z));
        }
        ...
        if (this._vertAttrFlags.uv0) {
          attrs.push(_vertAttrsCache.uv0.set( p.size.x, p.rotation.x));
        }
        if (this._vertAttrFlags.color) {
          attrs.push(_vertAttrsCache.color.set( p.color.r, p.color.g, p.color.b, p.color.a));
        }
        // TODO: other attrs.
        ...
        if (this._vertAttrFlags.color0) {
          attrs.push(_vertAttrsCache.color0.set( p.ultimateVelocity.x, p.ultimateVelocity.y, p.ultimateVelocity.z));
        }
