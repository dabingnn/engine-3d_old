...
function generateVerts(width, height, verts) {
  verts[0] = 0;
  verts[1] = 0;
  verts[2] = 0;
  verts[3] = 0;
...
export default class SpriteModelComponent extends Component {
  ...
  _getSpriteIndexBuffer() {
    let _indices = new Uint8Array([0, 1, 2, 3, 2, 1]);
    let _ib = new gfx.IndexBuffer(this._engine.device, gfx.INDEX_FMT_UINT8, gfx.USAGE_STATIC, _indices, _indices.length);
    return _ib;
  }

  _getSpriteVertexBuffer() {
    generateVerts(this._width, this._height, this._verts);
    let fmt = [];
    ...
    let _vb = new gfx.VertexBuffer(this._engine.device, new gfx.VertexFormat(fmt), gfx.USAGE_STATIC, this._verts, 4);
    return _vb;
  }
  ...
  set width(val) {
    this._width = val;
    this._ia._vertexBuffer = this._getSpriteVertexBuffer();
  }
