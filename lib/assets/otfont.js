...
export default class OpenTypeFont extends Font {
  ...
  _addTextToFontAtlas(text) {
    ...
    for (let i = 0; i < text.length; ++i) {
      ...
      if (this._glyphs[charCode] !== undefined) {
        ...
        if (bin !== null) {
          ...
          otGlyph.draw(ctx, bin.x - bbox.x1 * this._fontScale + this._padding / 2, bin.y + bbox.y2 * this._fontScale + this._padding / 2, this._size); // ?
        } else {
          console.error('font glyph pack failed.');
        }
      }
    }
    ...
    if (fontAtlasDirty) {
      ...
      //this._fontAtlas._loaded = true;
      this._fontAtlasReady = true;
    }
  }
    ...
    for (let i = 0; i < text.length; ++i) {
      ...
      if (this._glyphs[charCode] !== undefined) {
        ...
        if (refCount === 0) {
          ...
          this._packCanvas.clearRect(bin.x, bin.y, bin.width, bin.height);
          fontAtlasDirty = true;
        }
      } else {
