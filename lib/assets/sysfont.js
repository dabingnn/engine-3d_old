...
function _wrapText(text, style) {
  ...
  for (let i = 0; i < lines.length; ++i) {
    ...
    for (let j = 0; j < words.length; ++j) {
      ...
      const wordSpaceWidth = wordWidth + ctx.measureText(' ').width;
      if ( j === 0 || wordSpaceWidth > spaceLeft) {
        if (j > 0) {
          result += '\n';
        }
        result += words[j];
...
export default class SystemFont extends Asset {
  ...
  _updateTexture() {
    ...
    for (let i = 0; i < lines.length; ++i) {
      ...
      // HACK: no good idea to figure out glyph's ascent. use 0.75 * fontSize instead. we can also tweak value by topPadding.
      cursorY = i * Math.max(this._style.lineHeight, this._style.fontSize) + this._style.fontSize * 0.75 + this._style.topPadding;
