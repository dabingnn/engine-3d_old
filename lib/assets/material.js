...
export default class Material extends Asset {
  ...
  _updateEffectInst() {
    ...
    this._props = props;
    let defsInst = _objArrayClone(this._effect.defines);
    if (this._effectInst) {
      this._effectInst.clear();
      this._effectInst._techniques = techniques;
      this._effectInst._properties = props;
      this._effectInst._defines = defsInst;
    } else {
      this._effectInst = new renderer.Effect(techniques, props, defsInst);
    }
  }
  ...
  set blendType(val) {
    ...
    let mainTech = this._effectInst.getTechnique('opaque');
