...
function _generateSimpleSpriteVerts(out, sprite, width, height) {
  ...
  vec3.set(out._positions[0], 0, 0, 0);
  vec3.set(out._positions[1], width, 0, 0);
  vec3.set(out._positions[2], 0, height, 0);
  vec3.set(out._positions[3], width, height, 0);
...
class _RenderData extends _RenderDataInterface {
  constructor(owner) {
    super();
    this._owner = owner;
  }
...
export default class SpriteRenderData {
  constructor() {
  ...
  set sprite(val) {
    ...
    this._renderData._batchKey = val;
    this._vertexDataDirty = true;
  }
  ...
  updateModelData() {
    if (this._vertexDataDirty) {
      ...
      if (this._type === enums.SPRITE_SIMPLE) {
        _generateSimpleSpriteVerts(this, this._sprite, this._width, this._height);
      } else if (this._type == enums.SPRITE_SLICED) {
        _generateSlicedSpriteVerts(this, this._sprite, this._width, this._height);
      } else {
      }
    }
  }
