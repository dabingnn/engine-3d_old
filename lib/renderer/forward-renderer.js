...
export default class ForwardRenderer extends renderer.Base {
  ...
  _updateShaderOptions(items) {
    for (let i = 0; i < items.length; ++i) {
      ...
      options.directionalLightNum = Math.min(4, this._directionalLights.length);
      options.pointLightNum = Math.min(4, this._pointLights.length);
      options.spotLightNum = Math.min(4, this._spotLights.length);

      options.shadowLightNum = Math.min(4, this._shadowLights.length);
