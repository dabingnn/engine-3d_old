...
export default class ForwardRenderer extends renderer.Base {
  ...
  _2dStage(view, items) {
    ...
    // todo: this is a hack, use same sorting key as opaque, need to be changed later
    for (let i = 0; i < items.length; ++i) {
      let item = items.data[i];
  ...
  _drawDynamic(item) {
    ...
    if (model instanceof LineModel) {
      model.lines.forEach(l => {
        ...
        //
        let idx = i * 6;
        _lineData[idx] = l.start.x;
        _lineData[idx+1] = l.start.y;
        _lineData[idx+2] = l.start.z;
        _lineData[idx+3] = l.color.r;
        _lineData[idx+4] = l.color.g;
        _lineData[idx+5] = l.color.b;

        _lineData[idx+6] = l.end.x;
        _lineData[idx+7] = l.end.y;
        _lineData[idx+8] = l.end.z;
        _lineData[idx+9] = l.color.r;
        _lineData[idx+10] = l.color.g;
        _lineData[idx+11] = l.color.b;
