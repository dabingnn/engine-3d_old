import { vec3, mat4, color3 } from 'vmath';
...
export default class ForwardRenderer extends renderer.Base {
  ...
  updateLights(scene) {
    color3.array(this._sceneAmbient, scene._sceneAmbient);
  ...
  _opaqueStage(camera, items) {
    ...
    for (let i = 0; i < items.length; ++i) {
      let item = items.data[i];
      item.key = programLib.getKey(
        item.technique._passes[0]._programName,
        item.effect._options
      );
    }
  ...
  _transparentStage(camera, items) {
    ...
    for (let i = 0; i < items.length; ++i) {
      ...
      item.key = vec3.dot(_v3_tmp1, _camFwd);
    }
