...
export default class ForwardRenderer extends renderer.Base {
  ...
  updateLights(scene) {
    ...
    for (let i = 0; i < lights.length; ++i) {
      ...
      if (light.castShadow) {
        this._shadowMap.setupShadowMap(light._node);
      }
    }
  }
  ...
  render(scene) {
    ...
    this._render(this._shadowMap.getShadowView(), scene);
  ...
  _shadowStage(view, items) {
    ...
    this._device.setUniform('proj', mat4.array(_a16_proj, view._matProj));
    this._device.setUniform('viewProj', mat4.array(_a16_viewProj, view._matViewProj));
    this._device.setUniform('lightSpaceMatrix', this._shadowMap.getViewProjMat());
    this._shadowMap.getViewProjMat();
