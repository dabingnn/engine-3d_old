...
export default class ForwardRenderer extends renderer.Base {
  constructor(device, builtin) {
    ...
    // setup shadowmap
    this._shadowMap = new ShadowMap(device);
  ...
  updateLights(scene) {
    ...
    for (let i = 0; i < lights.length; ++i) {
      ...
      if (light.castShadow) {
        this._shadowMap.setupShadowMap(light);
      }
    }
  }
  ...
  render(scene) {
    ...
    if (this._shadowMap.generateShadow) {
      // render shadermap
      this._shadowMap.beginShadowMap();
      // TODO: should compute light shadow view-proj matrix based on current camera.
      this._render(this._shadowMap.getShadowView(scene._cameras.data[0]), scene);
      this._shadowMap.endShadowMap();
      if (this._shadowMap.enableBlur) {
        this._blur.setupBlur(this._shadowMap.getShadowMap());
        this._blur.processBlur();
      }
    }
  ...
  _shadowStage(view, items) {
    ...
    // update rendering
    this._shadowMap.submitShadowStageUniform();
  ...
  _opaqueStage(view, items) {
    ...
    this._device.setUniform('lightViewProjMatrix', this._shadowMap.getViewProjMat());

    // update rendering
    if (this._shadowMap.generateShadow) {
      this._shadowMap.submitOtherStageUniform();
      // TODO: set shadowmap texture to slot 10 temperarily, we should remove specify slot when set textures
      if (this._shadowMap.enableBlur) {
        this._device.setTexture('shadowMap', this._blur.getBluredTexture(), 10);
      } else {
        this._device.setTexture('shadowMap', this._shadowMap.getShadowMap(), 10);
      }
    }
    this._submitLightUniforms();
    this._updateShaderOptions(items);
