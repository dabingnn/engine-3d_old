...
export default class ForwardRenderer extends renderer.Base {
  constructor(device, builtin) {
    ...
    this._dynamicIAs = [];
    this._dynamicIAs.push(this._initBufferIA(320, 2048));
    this._dynamicIAs.push(this._initBufferIA(320, 2048));
  }
  ...
  _2dStage(view, items) {
    ...
    // draw it
    let iaIndex = 0;
    let iaCount = this._dynamicIAs.length;
    for (let i = 0; i < items.length; ++i) {
      let item = items.data[i];
      let data = item.ia;
      let ia = this._dynamicIAs[iaIndex];
      ia._vertexBuffer.update(0, data.vertices);
      ia._indexBuffer.update(0, data.indices);
      ia._start = 0;
      ia._count = data.indices.length;
      item.ia = ia;
      iaIndex = (iaIndex + 1) % iaCount;
      this._draw(item);
    }
  }
