...
export default class ForwardRenderer extends renderer.Base {
  ...
  _updateShaderOptions(items) {
    for (let i = 0; i < items.length; ++i) {
      ...
      options.NUM_DIR_LIGHTS = Math.min(4, this._directionalLights.length);
      options.NUM_POINT_LIGHTS = Math.min(4, this._pointLights.length);
      options.NUM_SPOT_LIGHTS = Math.min(4, this._spotLights.length);

      options.NUM_SHADOW_LIGHTS = Math.min(4, this._shadowLights.length);

      if (model instanceof SkinningModel) {
        model.updateMatrices();
        options.USE_SKINNING = true;
      } else {
        options.USE_SKINNING = false;
      }
    }
  }
  ...
  _opaqueStage(view, items) {
    ...
    for (let i = 0; i < items.length; ++i) {
      ...
      for (let index = 0; index < this._shadowLights.length; ++index) {
        let light = this._shadowLights[index];
        this._device.setTexture(`shadowMap_${index}`, light.shadowMap, this._allocTextuerUnit());
      }
