...
export default class BaseRenderer {
  ...
  _render(view, scene) {
    ...
    for (let i = 0; i < scene._models.length; ++i) {
      ...
      // TODO: HACK: filter model by camera
      if (model._cameraID != -1 && model._cameraID != view._cameraID)
        continue;

      // frustum culling
      if (model._boundingBox !== null) { // if model does not have boundingBox, skip culling.
        model._node.getWorldMatrix(_m4_tmp);
        // model._boundingBox.toWorldSpace(_m4_tmp);
        // if (model._boundingBox.isInFrustum(view._frustumPlanes) === false) {
        box.setTransform(model._boundingBox, _m4_tmp, model._bbModelSpace);
        if (box.isInFrustum(model._boundingBox, view._frustumPlanes) === false) {
