...
export default class ParticleBatchModel extends Model {
  ...
  enableStretchedBillboard() {
    if (this._vertAttrs.find(attr => attr.name === gfx.ATTR_COLOR0) === undefined) {
      this._vertAttrs.push({ name: gfx.ATTR_COLOR0, type: gfx.ATTR_TYPE_FLOAT32, num: 4 });
  ...
  addParticleVertexData(index, pvdata) {
    ...
    for (let i = 0; i < this._vertAttrs.length; ++i) {
      ...
      if (curAttr.num === 1) {
        ...
        this._vdataF32[offset] = pvdata[i].r;
        this._vdataF32[offset + 1] = pvdata[i].g;
        this._vdataF32[offset + 2] = pvdata[i].b;
        this._vdataF32[offset + 3] = pvdata[i].a;
        offset += 4;
      } else {
