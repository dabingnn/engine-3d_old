...
function createComponent(app, ent, info, callback) {
  ...
  if (info.type === 'Light') {
    ...
    if ( props.type === 'ortho') {
      comp.projection = enums.PROJ_ORTHO;
    } else {
      comp.projection = enums.PROJ_PERSPECTIVE;
    }
    comp.fov = props.fov;
    comp.orthoHeight = props.orthoHeight;
    comp.near = props.near;
    comp.far = props.far;
    comp.color = color4.new(props.color[0], props.color[1], props.color[2], props.color[3]);
    comp.clearFlags = props.clearFlags;
    comp.rect = {
      x: props.rect[0],
      y: props.rect[1],
      w: props.rect[2],
    ...
    if (props.type === 'simple') {
      comp.type = enums.SPRITE_SIMPLE;
    } else if (props.type === 'sliced') {
      comp.type = enums.SPRITE_SLICED;
    }

    if (props.sprite) {
      app.assets.load(props.sprite, (err, asset) => {
        if (err) {
          console.error(err);
          callback(null, comp);
          return;
    ...
    comp.label = props.text;
    comp.fontSize = props.fontSize;
    switch(props.horizontalAlign) {
      case 'left':
        comp.horizontalAlign = enums.TEXT_ALIGN_LEFT;
        break;

      case 'center':
        comp.horizontalAlign = enums.TEXT_ALIGN_CENTER;
        break;

      case 'right':
        comp.horizontalAlign = enums.TEXT_ALIGN_RIGHT;
        break;
    }
    switch(props.verticalAlign) {
      case 'top':
        comp.verticalAlign = enums.TEXT_ALIGN_TOP;
        break;

      case 'center':
        comp.verticalAlign = enums.TEXT_ALIGN_CENTER;
        break;

      case 'bottom':
        comp.verticalAlign = enums.TEXT_ALIGN_BOTTOM;
        break;
    }
    comp.color = color4.new(
      props.color[0],
    ...
    app.assets.load(props.font, (err, font) => {
      if (err) {
        console.error(err);
        callback(null, comp);
        return;
    ...
    // TODO: should use schema and serailized techinque in here
...
function preloadAssets(app, json, callback) {
  ...
  for (let i = 0; i < entInfos.length; ++i) {
    ...
    for (let c = 0; c < entInfo.components.length; ++c) {
      ...
      } else if (compInfo.type === 'Sprite') {
        if (props.sprite !== null) {
          let uuid = _mainUuid(props.sprite);
          if (uuids.indexOf(uuid) === -1) {
            uuids.push(uuid);
          }
        }
      } else if (compInfo.type === 'Label') {
        if (props.font !== null) {
          let uuid = _mainUuid(props.font);
          if (uuids.indexOf(uuid) === -1) {
            uuids.push(uuid);
          }
        }
