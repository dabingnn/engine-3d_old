 ...
 * @param {object} info
 */
function createComponent(app, ent, info, level = null) {
  ...
  let comp = app._createComp(ctor, ent, info.properties, level);
 ...
 * @param {object} info
 */
function createEntity(app, info, level = null) {
  ...
  for (let i = 0; i < info.components.length; ++i) {
    ...
    ent._comps[i] = createComponent(app, ent, compInfo, level);
  }
 ...
 */
function createPrefab(app, json, modifications, level = null) {
