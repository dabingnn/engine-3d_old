...
function createComponent(app, ent, info, callback) {
  ...
  if (info.type === 'Light') {
    ...
    for (let i = 0; i < props.materials.length; ++i) {
      ...
      tasks.push(done => {
        app.assets.load(propMat, (err, asset) => {
          comp.material = asset;
          done();
        });
      });
...
function createEntity(app, info, callback) {
  ...
  for (let i = 0; i < info.components.length; ++i) {
    ...
    ent._comps[i] = createComponent(app, ent, compInfo, (err) => {
      if (err) {
...
function createPrefab(app, json, callback) {
  ...
  for (let i = 0; i < entInfos.length; ++i) {
    ...
    if (entInfo.prefab) {
      ...
      entities[i] = createEntity(app, entInfo, (err) => {
        if (err) {
...
function preloadAssets(app, json, callback) {
  ...
  for (let i = 0; i < entInfos.length; ++i) {
    ...
    for (let j = 0; j < entInfo.components.length; ++j) {
      const compInfo = entInfo.components[i];
      const props = compInfo.properties;

      if (compInfo.type === 'Model') {
        let uuid;

        // materials
        for (let i = 0; i < props.materials.length; ++i) {
          uuid = props.materials[i];
