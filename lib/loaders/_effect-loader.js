...
function createEffect(json) {
  ...
  for (let i = 0; i < json.techniques.length; ++i) {
    ...
    for (let j = 0; j < jsonTech.passes.length; ++j) {
      ...
      if(jsonPass.depthTest !== undefined && jsonPass.depthWrite !== undefined) {
        passes[j].setDepth(jsonPass.depthTest, jsonPass.depthWrite);
      }
      if (jsonPass.cullMode !== undefined) {
  ...
  effectAsset._effect = new renderer.Effect(techniques, props, json.defines);
