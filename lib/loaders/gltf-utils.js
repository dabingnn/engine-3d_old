...
import Skinning from '../assets/skinning';
...
function createMesh(device, gltf, bin, index) {
  ...
  for (let i = 0; i < gltfMesh.primitives.length; ++i) {
    ...
    meshAsset._subMeshes[i] = new renderer.Mesh(vb, ib);
  }
...
function createSkinning(gltf, bin, index) {
  ...
  let skinningAsset = new Skinning();

  skinningAsset._name = gltfSkin.name;
  skinningAsset._jointIndices = gltfSkin.joints;
  skinningAsset._bindposes = bindposes;

  return skinningAsset;
}
