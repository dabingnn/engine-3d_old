...
function getLerpFunc(type) {
  if (type === 'number') {
    return (out, from, to, t) => { out = ((to - from) * t) + from; return out; };
  } else if (type === 'vec2') {
    return vec2.lerp;
  } else if (type === 'vec3') {
    return vec3.lerp;
    ...
    for (let i = 0; i < this.tracks.length; i++) {
      ...
      for (let j = 0; j < track.keys.length; j++) {
        ...
        if (key.startTime <= time && key.endTime >= time) {
          ...
          track.target[track.property] = track.lerp(track.target[track.property], key.from, key.to, eased);

          break;
        } else if (key.startTime > time) {
          track.lerp(track.target[track.property], key.from, key.to, 0);
        } else if (key.endTime < time) {
          track.lerp(track.target[track.property], key.from, key.to, 1);
        }
      }
    }
