...
/**
 * box-frustum intersect, faster but has false positives
 * in some rare case, read more here:
...
/**
 * sphere-frustum intersect, faster but has false positives
 * in some rare case, read more here:
...
let sphere_frustum = function (sphere, frustum) {
  for (let i = 0; i < 6; i++)
    // frustum plane normal points to the inside
    if (sphere_plane(sphere, frustum[i]) == 1)
      return false; // completely outside
  return true;
};
 ...
 * @return {boolean}
 */
let sphere_frustum_accurate = function (sphere, frustum) {
  let result = 0, intersects = false;
  for (let i = 0; i < 6; i++) {
    result = sphere_plane(sphere, frustum[i]);
    // frustum plane normal points to the inside
    if (result == 1) return false; // completely outside
    else if (result == -1) intersects = true;
  }
