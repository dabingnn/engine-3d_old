...
export default class PbrMaterial extends Material {
  constructor() {
    ...
    let mainTech = new Technique(
      ...
      [
        ...
        { name: 'irradianceMap', type: renderer.PARAM_TEXTURE_CUBE},
        { name: 'prefilterMap', type: renderer.PARAM_TEXTURE_CUBE},
  ...
  set irradianceMap(val) {
    this._irradianceMap = val;
    this._effect.setValue('irradianceMap', val._texture);
  }

  set prefilterMap(val) {
    this._prefilterMap = val;
    this._effect.setValue('prefilterMap', val._texture);
  }
  ...
  set useTexLod(val) {
    if (!window.app.device.ext('EXT_shader_texture_lod')) {
      this._effect.setOption('useTexLod', false);
      console.warn("can not support texture lod.");
      return;
    }
