...
export default class PhongMaterial extends Material {
  constructor(values = {}) {
    ...
    this.emissive = color3.create();

    this.ambient = color3.create();
    this.opacity = 1.0;
    this.alphaTestRef = 0.0;

    this.specular = color3.create();
  ...
  set useGlossinessTexture(val) {
    this._effect.setOption('useGlossinessTexture', val);
  }

  set specular(val) {
    this._effect.setValue('specularValue', val);
  }

  set glossiness(val) {
    this._effect.setValue('glossinessValue', val);
  }
  ...
  set glossinessTexture(val) {
    this._effect.setValue('glossinessTexture', val);
  }
}